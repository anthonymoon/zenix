{
  ...
  detectMatchingDisks = {
    ...
    groupBySimilarSize = disks: let
      # Calculate size groups with tolerance
      getGroupKey = diskInfo: let
    ...
    findBestGroup = groups: let
      # Score each group
      scoreGroup = groupDisks: let
  ...
  detectDiskBySerial = serialNumber: let
    allDisks = getAllDisks {};
    matchingDisk =
      lib.findFirst (
        disk: let
