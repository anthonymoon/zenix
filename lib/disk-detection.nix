{ lib, pkgs, ... }: rec {
  ...
  getDiskInfo = disk:
    let
      ...
      realDisk = if lib.hasPrefix "/dev/disk/by-id/" disk then
        builtins.readLink disk
      else
        disk;

      # Extract just the device name for lsblk
      deviceName = builtins.baseNameOf realDisk;
    in builtins.fromJSON (builtins.readFile
  ...
  getAllDisks = { excludeRemovable ? true, excludeLoop ? true
    , excludePartitions ? true, excludeOptical ? true, }:
    let
      # Read all symlinks from /dev/disk/by-id
      byIdPath = "/dev/disk/by-id";
      allEntries =
        if builtins.pathExists byIdPath then builtins.readDir byIdPath else { };
      ...
      diskPaths =
        lib.mapAttrsToList (name: _: "${byIdPath}/${name}") diskEntries;
      ...
      filteredDisks = builtins.filter (disk:
        ...
        in (!excludePartitions || !(lib.hasInfix "-part" name
          || builtins.match ".*[0-9]$" name != null)) &&
        # Exclude loop devices
        (!excludeLoop || !lib.hasPrefix "loop-" name) &&
        # Exclude optical drives
        (!excludeOptical || !(lib.hasInfix "cd" name || lib.hasInfix "dvd" name
          || lib.hasInfix "bd" name)) &&
  ...
  isRemovable = disk:
    let
      devicePath = if lib.hasPrefix "/dev/disk/by-id/" disk then
        builtins.readLink disk
      else
        disk;
      deviceName = builtins.baseNameOf devicePath;
      sysPath = "/sys/block/${deviceName}/removable";
    in builtins.pathExists sysPath && (builtins.readFile sysPath) == ''
      1
    '';

  # Filter disks based on criteria
  filterDisks = { disks ? getAllDisks { }, minSizeGB ? 20, maxSizeGB ? null
    , transport ? null,
    # "nvme", "sata", "usb", etc.
    rotational ? null,
    ...
    builtins.filter (disk:
      let
        ...
        validDisk = !hasError && info ? blockdevices
          && builtins.length info.blockdevices > 0;
      ...
      in validDisk && notExcluded && (minSizeGB == null || sizeGB >= minSizeGB)
      ...
      && (model == null
        || (diskData ? model && builtins.match model diskData.model != null)))
    disks;

  # Smart primary disk detection with scoring
  detectPrimaryDisk = args@{ preferNvme ? true, preferSSD ? true, minSizeGB ? 20
    , excludeDisks ? [ ], fallbackToAny ? true, ... }:
    let
      ...
      # Get the best disk
      bestDisk = if builtins.length sortedDisks > 0 then
        (builtins.head sortedDisks).disk
      else
        null;
    in if bestDisk != null then
      bestDisk
    else if fallbackToAny then
    # Try to get any disk if no good candidates
      let allDisks = getAllDisks { };
      in if builtins.length allDisks > 0 then
        builtins.head allDisks
      else
        throw "No suitable disk found with criteria: ${builtins.toJSON args}"
    else
      throw "No suitable disk found with criteria: ${builtins.toJSON args}";

  # Detect matching disks for RAID configurations
  detectMatchingDisks =
    { count ? 2, sizeTolerancePercent ? 5, preferSameModel ? true, ... }@args:
    let
      allDisks = filterDisks
        (removeAttrs args [ "count" "sizeTolerancePercent" "preferSameModel" ]);
        ...
        let
          ...
          getGroupKey = diskInfo:
            let
              ...
              validDisk = !hasError && diskInfo.info ? blockdevices
                && builtins.length diskInfo.info.blockdevices > 0;

              diskData = if validDisk then
                builtins.head diskInfo.info.blockdevices
              else
                { };
              sizeBytes = if diskData ? size then diskData.size else 0;
              sizeGB = sizeBytes / (1024 * 1024 * 1024);
              ...
              groupKey =
                toString (builtins.floor (sizeGB / groupSize) * groupSize);
            in if validDisk then groupKey else "invalid";

          # Group by size key
          grouped = lib.groupBy getGroupKey disks;
          # Remove invalid disks group
        in removeAttrs grouped [ "invalid" ];
            ...
            let
              ...
              nvmeCount = builtins.length (builtins.filter (d:
                let
                  ...
                  validDisk = !hasError && d.info ? blockdevices
                    && builtins.length d.info.blockdevices > 0;
                  diskData = if validDisk then
                    builtins.head d.info.blockdevices
                  else
                    { };
                in validDisk && diskData ? tran && diskData.tran == "nvme")
                groupDisks);
              ...
              ssdCount = builtins.length (builtins.filter (d:
                let
                  ...
                  diskData = if validDisk then
                    ...
                    { };
                in validDisk && diskData ? rota && diskData.rota == "0")
                groupDisks);

              score = (if hasEnoughDisks then 1000 else 0) + (nvmeCount * 100)
                + (ssdCount * 50) + groupSize;
            in { inherit groupDisks score; };

          scoredGroups =
            lib.mapAttrsToList (key: disks: scoreGroup disks) groups;
          sortedGroups = lib.sort (a: b: a.score > b.score) scoredGroups;
        in if builtins.length sortedGroups > 0 then
          (builtins.head sortedGroups).groupDisks
        else
          [ ];
    ...
    in if builtins.length selectedDisks >= count then
      selectedDisks
    else
      throw "Could not find ${toString count} matching disks with criteria: ${
        builtins.toJSON args
      }";

  # Detect disk by pattern matching
  detectDiskByPattern = { patterns ? [ ],
    # List of regex patterns to match against /dev/disk/by-id/
    fallback ? null, }:
    let
      allDisks = getAllDisks { };

      matchingDisks = builtins.filter
        (disk: lib.any (pattern: builtins.match pattern disk != null) patterns)
        allDisks;
    in if builtins.length matchingDisks > 0 then
      builtins.head matchingDisks
    else if fallback != null then
      fallback
    else
      throw "No disk matching patterns: ${builtins.toJSON patterns}";
      ...
      matchingDisk = lib.findFirst (disk:
        let
          ...
          validDisk = !hasError && info ? blockdevices
            && builtins.length info.blockdevices > 0;
          diskData = if validDisk then builtins.head info.blockdevices else { };
        in validDisk && diskData ? serial && diskData.serial == serialNumber)
        null allDisks;
    in if matchingDisk != null then
      matchingDisk
    else
      throw "No disk found with serial number: ${serialNumber}";

  # Generate disk detection at build time for static configurations
  generateDiskConfig =
    { hostName, preferredPatterns ? [ ], fallbackDetection ? { }, }:
    let
      # Try pattern matching first
      patternResult = if builtins.length preferredPatterns > 0 then
        try (detectDiskByPattern { patterns = preferredPatterns; }) null
      else
        null;

      # Fall back to auto-detection
      autoResult = if patternResult == null then
        detectPrimaryDisk fallbackDetection
      else
        patternResult;
    in autoResult;
}
