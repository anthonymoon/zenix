...
let
  ...
  # CPU Detection
  cpuVendor = 
    if lib.strings.hasInfix "GenuineIntel" cpuinfo then "intel"
    else if lib.strings.hasInfix "AuthenticAMD" cpuinfo then "amd"
    else "generic";
  ...
  isVirtual = lib.strings.hasInfix "hypervisor" cpuinfo ||
              builtins.pathExists /sys/hypervisor/type;
  
  # VM Type Detection
  vmType = 
    if !isVirtual then null
    else if builtins.pathExists /sys/devices/virtual/dmi/id/sys_vendor then
      let vendor = lib.strings.removeSuffix "\n" (builtins.readFile /sys/devices/virtual/dmi/id/sys_vendor);
      in
        if vendor == "QEMU" || vendor == "KVM" then "qemu-kvm"
        else if vendor == "VMware, Inc." then "vmware"
        else if vendor == "innotek GmbH" then "virtualbox"
        else if vendor == "Microsoft Corporation" then "hyperv"
        else if vendor == "Xen" then "xen"
        else "generic-vm"
    else "generic-vm";
in
{
