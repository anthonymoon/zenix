...
let
  ...
  cpuVendor =
    if cpuinfo != "" && lib.strings.hasInfix "GenuineIntel" cpuinfo then
      "intel"
    else if cpuinfo != "" && lib.strings.hasInfix "AuthenticAMD" cpuinfo then
      "amd"
    else
      "generic";

  # GPU Detection (check for kernel modules) with safe path checking
  hasNvidia = safePathExists /sys/module/nvidia || safePathExists /dev/nvidia0;
  hasAmdGpu = safePathExists /sys/module/amdgpu
  ...
  isVirtual = (cpuinfo != "" && lib.strings.hasInfix "hypervisor" cpuinfo)
    || safePathExists /sys/hypervisor/type;

  # VM Type Detection with safe file operations
  vmType = if !isVirtual then
    null
  else if safePathExists /sys/devices/virtual/dmi/id/sys_vendor then
    let
      vendor = lib.strings.removeSuffix "\n"
        (safeReadFile /sys/devices/virtual/dmi/id/sys_vendor "unknown");
    in if vendor == "QEMU" || vendor == "KVM" then
      "qemu-kvm"
    else if vendor == "VMware, Inc." then
      "vmware"
    else if vendor == "innotek GmbH" then
      "virtualbox"
    else if vendor == "Microsoft Corporation" then
      "hyperv"
    else if vendor == "Xen" then
      "xen"
    else
      "generic-vm"
  else
...
in {
  ...
  system.nixos.tags = [ "cpu:${cpuVendor}" ]
